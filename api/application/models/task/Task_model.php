...
class Task_model extends CI_Model{
 ...
 // kullanıcının vermiş olduğu task bilgisini geri  döndürür.   
    ...
    // Seçilen taskı alan kullanıcıları döndürür.
    function select_task_userk_info($task_id){
       $arr=["0","1"];
       $this->db->select('b.*,u.ad,u.soyad,u.is_company');
       $this->db->from('tb_buy_task b');
       $this->db->join('tb_uye u', 'u.uye_id= b.user_id', 'left');
       $this->db->where("b.task_id",$task_id);
       $this->db->where_in("b.work_status",$arr);
       $this->db->group_by("b.id");
       $que=$this->db->get();
       $res=$que->result(); 
	...
	function get_all_tasks(){
		...
		$this->db->select('t.*,u.img_url as user_img_url');
		$this->db->from('tb_task t');
		$this->db->join('tb_uye u', 'u.uye_id= t.user_id', 'left');
